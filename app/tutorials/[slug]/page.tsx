...
import { generateMetadata as genMeta, webpageSchema, breadcrumbSchema, howToSchema, renderJsonLd } from '@/lib/seo'
...
export async function generateMetadata({
  ...
  return genMeta({
    title: `${tutorial.title} - Step-by-Step Guide`,
...
export default async function TutorialDetailPage({
  ...
  const tutorialUrl = `${siteConfig.url}/tutorials/${tutorial.slug}`
          ...
          <article>
            ...
            {tutorial.prerequisites.length > 0 && (
              <div className="mb-10 rounded-2xl border border-amber-500/20 bg-amber-500/5 p-5">
                <h2 className="mb-3 flex items-center gap-2 text-sm font-bold text-amber-600 dark:text-amber-400">
                  <span className="grid size-5 place-items-center rounded-md bg-amber-500/10 text-[10px]" aria-hidden="true">!</span>
                  Prerequisites
                </h2>
                <ul className="space-y-1.5">
                  {tutorial.prerequisites.map((p) => (
                    <li key={p} className="flex items-center gap-2 text-sm text-muted-foreground">
                      <span className="size-1.5 shrink-0 rounded-full bg-amber-500/50" aria-hidden="true" />
                      {p.replace(/-/g, ' ')}
                    </li>
                  ))}
                </ul>
              ...
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <p className="text-sm font-semibold">
                    {tutorial.steps.length} steps - Tutorial complete
                  </p>
                  <p className="mt-1 text-xs text-muted-foreground">
                    Have questions? Reach out on our support channels.
                  </p>
                </div>
                <div className="flex flex-wrap gap-1.5" role="list" aria-label="Tutorial tags">
